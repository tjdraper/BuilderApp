...
function scanpages($pattern, $final_includes_before, $final_includes_after, $minify, $single_variables, $variable_pairs) {
	...
	foreach (glob($pattern) as $filename) {
		...
		if (is_dir($filename) == 1) {
			mkdir('../' . $dir);
			scanpages($filename . '/*', $final_includes_before, $final_includes_after, $minify, $single_variables, $variable_pairs);
		} else {			
			// Set the file
			$file = fopen('../' . $dir,'w');
			...
			foreach ($single_variables as $variable_key => $variable_value) {
				$content =  preg_replace('{{{' . $variable_key . '}}}', $variable_value, $content);
			}

			// Parse Variable Pairs
			foreach ($variable_pairs as $variable_key) {
				preg_match('^{{' . $variable_key . '}}(.*){{/' . $variable_key . '}}^', $content, $variable_value);
				$content =  preg_replace('^{{' . $variable_key . '}}(.*){{/' . $variable_key . '}}^', '', $content);
				$content = preg_replace('{{{' . $variable_key . '}}}', $variable_value[1], $content);
			}
...
scanpages('pages/*', $final_includes_before, $final_includes_after, $minify, $single_variables, $variable_pairs);
